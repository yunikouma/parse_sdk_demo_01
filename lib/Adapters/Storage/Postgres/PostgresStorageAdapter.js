...
class PostgresStorageAdapter {
  ...
  setIndexesWithSchemaFormat(className, submittedIndexes, existingIndexes = {}, fields, conn) {
    ...
    return deletePromise.then(() => insertPromise).then(() => this._ensureSchemaCollectionExists()).then(() => {
  ...
  createTable(className, schema, conn) {
    ...
    return this._ensureSchemaCollectionExists(conn).then(() => conn.none(qs, values)).catch(error => {
    ...
    return this._client.tx("addFieldIfNotExists", t => {
      ...
      return promise.then(() => {
        ...
        if (result[0]) {
          ...
          return t.none('UPDATE "_SCHEMA" SET "schema"=jsonb_set("schema", $<path>, $<type>)  WHERE "className"=$<className>', { path, type, className });
        }
      });
    });
  }
  ...
  deleteAllClasses() {
    ...
    return this._client.any('SELECT * FROM "_SCHEMA"').then(results => {
  ...
  deleteFields(className, schema, fieldNames) {
    ...
    return Promise.resolve().then(() => {
      ...
      const doBatch = t => {
        const batch = [t.none('UPDATE "_SCHEMA" SET "schema"=$<schema> WHERE "className"=$<className>', { schema, className })];
        if (values.length > 1) {
  ...
  getAllClasses() {
    return this._ensureSchemaCollectionExists().then(() => this._client.map('SELECT * FROM "_SCHEMA"', null, row => _extends({ className: row.className }, row.schema))).then(res => res.map(toParseSchema));
  }
  ...
  distinct(className, schema, query, fieldName) {
    ...
    debug(qs, values);
    return this._client.any(qs, values).catch(() => []).then(results => {
      if (fieldName.indexOf('.') === -1) {
  ...
  aggregate(className, schema, pipeline) {
    ...
    debug(qs, values);
    return this._client.any(qs, values).then(results => results.map(object => this.postgresObjectToParseObject(className, object, schema))).then(results => {
      if (countField) {
