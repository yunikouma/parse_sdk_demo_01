...
class MongoStorageAdapter {
  ...
  createClass(className, schema) {
    ...
    return this._schemaCollection().then(schemaCollection => schemaCollection._collection.insertOne(mongoObject)).then(result => _MongoSchemaCollection2.default._TESTmongoSchemaToParseSchema(result.ops[0])).catch(error => {
      if (error.code === 11000) {
  ...
  find(className, schema, query, { skip, limit, sort, keys, readPreference }) {
    ...
    return this.createTextIndexesIfNeeded(className, query).then(() => this._adaptiveCollection(className)).then(collection => collection.find(mongoWhere, {
      skip,
      limit,
      sort: mongoSort,
      keys: mongoKeys,
  ...
  distinct(className, schema, query, fieldName) {
    schema = convertParseSchemaToMongoSchema(schema);
    return this._adaptiveCollection(className).then(collection => collection.distinct(fieldName, (0, _MongoTransform.transformWhere)(className, query, schema)));
  }
  ...
  createTextIndexesIfNeeded(className, query) {
    for (const fieldName in query) {
      ...
      return this.createIndex(className, index).catch(error => {
        if (error.code === 85) {
